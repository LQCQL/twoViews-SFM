...
//    std::string fname2(argv[2]);
    std::string fname1("./data/test1.jpg");
    std::string fname2("./data/test2.jpg");
     ...
     * 提取相机焦距
     */
    float f1 = extract_focal_len(fname1);
    float f2 = extract_focal_len(fname2);
    ...
    {
        std::cerr<<"Error to find correct camera poses!"<< std::endl;
        return -1;
    }
    ...
    std::ofstream out("./data/test_ba.txt");
    assert(out.is_open());
